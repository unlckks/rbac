<template>
  <div class="app-container">
    ...
    <el-dialog :title="title" center width="50%" :visible.sync="addDialog"  v-if="addDialog">
      <sys-role-add @after="getByPage" @close="closeDialog"/>
    </el-dialog>
    <!-- 添加弹窗结束 -->

    <!-- 修改弹窗 -->
    <el-dialog :title="title" center width="50%" :visible.sync="updateDialog"  v-if="updateDialog">
      <sys-role-update @after="getByPage" :active-id="activeId" @close="closeDialog"/>
    </el-dialog>
    <!-- 修改弹窗结束 -->

    <!--分配权限开始-->
    <el-dialog :title="title" center width="50%" :visible.sync="selectRoleDialog"  v-if="selectRoleDialog">
      <sys-role-select-menu :active-id="activeId" @close="closeDialog"/>
    </el-dialog>
    <!--分配权限结束-->
  </div>
</template>
...
import SysRoleUpdate from "@/views/system/role/sys-role-update";
...
export default {
  ...
  methods:{
    ...
    //打开修改的弹出层
    toUpdate(row){
      this.title="修改【"+row.roleName+"】的角色";
      this.updateDialog=true;
      this.activeId=row.roleId;
    },
    //删除角色
    toDelete(row){
      this.$confirm('此操作会删除【'+row.roleName+'】该角色，是否继续?', '提示', {
    ...
    //打开分配权限的弹出层
    toSelectMenu(row){
      this.title="给【"+row.roleName+"】的角色分配权限";
