<template>
  <div class="app-container">
                  ...
                  <template slot-scope="{row}">
                    ...
                    <span class="el-dropdown-link">
                      ...
                      <el-dropdown-menu slot="dropdown">
                        ...
                        <el-dropdown-item>
                          <el-button type="text" @click="toDeleteDictType(row)" icon="el-icon-delete">删除</el-button>
                        </el-dropdown-item>
                      </el-dropdown-menu>
                    </el-dropdown>
                  </template>
          ...
          <el-card style="margin-top: 3px">
            ...
            <el-form>
              <el-form-item>
                ...
                <el-button size="small" type="primary" icon="el-icon-refresh" @click="toRefreshCache">同步缓存</el-button>
                <!--工具条结束-->
              </el-form-item>
            </el-form>
            <!--字典数据管理表格开始-->
              ...
              <span class="el-dropdown-link">
                <el-link icon="el-icon-s-operation" type="primary">操作<i class="el-icon-arrow-down el-icon--right"/>
                    ...
                    <el-dropdown-menu slot="dropdown">
                      ...
                      <el-dropdown-item>
                        <el-button type="text" @click="toDeleteDictData(row)" icon="el-icon-delete">删除</el-button>
                      </el-dropdown-item>
                    </el-dropdown-menu>
                  </el-dropdown>
                </template>
    ...
    <!--字典类型添加的弹出层开始-->
    <el-dialog :title="title" center width="50%" :visible.sync="addDictTypeDialog" v-if="addDictTypeDialog">
      <sys-dict-type-add @after="getDictTypeByPage" @close="closeDialog"/>
    </el-dialog>
    <!--字典类型添加的弹出层结束-->

    <!--字典类型修改的弹出层开始-->
    <el-dialog :title="title" center width="50%" :visible.sync="updateDictTypeDialog" v-if="updateDictTypeDialog">
      <sys-dict-type-update @after="getDictTypeByPage" :active-id="dictTypeActiveId" @close="closeDialog"/>
    </el-dialog>
    <!--字典类型修改的弹出层结束-->

    <!--字典数据添加的弹出层开始-->
    <el-dialog :title="title" center width="50%" :visible.sync="addDictDataDialog" v-if="addDictDataDialog">
      <sys-dict-data-add @after="queryDictDataByDictType" :active-type="currentDictType" @close="closeDialog"/>
    </el-dialog>
    <!--字典数据添加的弹出层结束-->

    <!--字典数据添加的弹出层开始-->
    <el-dialog :title="title" center width="50%" :visible.sync="updateDictDataDialog" v-if="updateDictDataDialog">
      <sys-dict-data-update @after="queryDictDataByDictType" :active-id="dictDataActiveId" @close="closeDialog"/>
    </el-dialog>
    <!--字典数据添加的弹出层结束-->
